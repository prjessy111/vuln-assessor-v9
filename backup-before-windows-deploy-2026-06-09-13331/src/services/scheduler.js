...
async function fetchCustomXmlViaWindows(targetServer) {
  ...
  const localXmlPath = getDailyUploadPath(hostname, 'script');
  const scriptMode = String(process.env.SCHEDULER_SCRIPT_MODE || 'fast').toLowerCase();
  const scriptArgs = scriptMode === 'full' ? '-Full' : '-Fast';
...
async function runScheduledDiagnosis(targetServer, options = {}) {
  ...
  try {
    ...
    if (!localDbPath && !scriptPath) {
      throw new Error(`no ${requestedSource} source data was collected`);
    }
    ...
    console.log(`[Scheduler] [${hostname}] diagnosis started: engine=${diagnosisEngine}, source=${runRecord.source_type}`);
